...
--- ARGV[3]   ---  过期小时上限
-------------------------------------------------------------
local expire_jp_prefix = "slotsdata:10016:h:s:{pool}:jackpotexpire";   -- jp_pool 对应的过期时间缓存 redis hashKey
local expire_bonus_prefix = "slotsdata:10016:h:s:{pool}:bonusexpire";        -- bonus_pool 对应的过期时间缓存 redis hashKey
...
for i = 1, #jackpotPool do
    ...
    local random_hour = math.random(tonumber(ARGV[2]), tonumber(ARGV[3]));
    -- 初始化: 不存在就进行初始化
    if (type(expire_time) == 'boolean') then
        redis.call("HSET", expire_jp_prefix, jackpotPool[i], tonumber(ARGV[1]) + random_hour * hour_mills);
...
for i = 1, #bonusPool do
    ...
    -- 初始化: 不存在就进行初始化
    if (type(expire_time) == 'boolean') then
        redis.call("HSET", expire_bonus_prefix, bonusPool[i], tonumber(ARGV[1]) + random_hour * hour_mills);
